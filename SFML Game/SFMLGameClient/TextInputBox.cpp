...
void TextInputBox::HandleLocalTextInput(sf::Event &events, bool eventFired, const sf::Input &input)
{
	...
	{
		if ((events.Text.Unicode < 128) && (events.Text.Unicode != 8) && (events.Text.Unicode != 13) && (tempBuffer.size() < (size_t)maxChars))//if the inputted text is in the unicode character set, and isnt delete, and isnt enter, and isnt larger than the limit
		{
