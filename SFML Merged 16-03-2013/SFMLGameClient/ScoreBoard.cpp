...
{
	//ALL THIS SHIT IS SUPER TEMPORARY UNTIL MOVING CAMERA IS IMPLEMENTED

	//Center the scoreboard (Yes Mark I know what you're gonna say, just ... it's here for now)
	scoreBoardPosition.x = (renderWindow.GetWidth()/2) - (frameSprite.GetSize().x/2);
	scoreBoardPosition.y = (renderWindow.GetHeight()/2) - (frameSprite.GetSize().y/2);
	frameSprite.SetPosition(scoreBoardPosition);
